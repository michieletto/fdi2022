...
author* load_authors(int *num_authors, char filepath[]) {
    FILE* f = fopen(filepath, "r");
    author *author_db = NULL;

    if (f == NULL)
        return NULL;
    
    fscanf(f, "%d", num_authors);
    
    author_db = (author*)malloc((*num_authors)*sizeof(author));

    for (int i = 0; i < *num_authors; i++)
    {   
        fscanf(f, "%s", author_db[i].name);
        fscanf(f, "%s", author_db[i].surname);
        fscanf(f, "%d", &author_db[i].day);
        fscanf(f, "%d", &author_db[i].month);
        fscanf(f, "%d", &author_db[i].year);
    }
...
bool save_authors(author author_db[], int num_authors, char filepath[]) {
    ...
    for (int i = 0; i < num_authors; i++)
    {
        fprintf(f, "%s ", author_db[i].name);
        fprintf(f, "%s ", author_db[i].surname);
        fprintf(f, "%d ", author_db[i].day);
        fprintf(f, "%d ", author_db[i].month);
        fprintf(f, "%d\n", author_db[i].year);
    }
...
int main () {
    ...
    author *authors = load_authors(&num_authors, "../data/authors.txt");

    // Test di acquisizione da utente di un autore
    //read_author(authors, &num_authors);

    //save_authors(authors, num_authors, "../data/authors.txt");
